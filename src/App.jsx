...
══════════════════════════════════════════════════════════ */
function DetailView() {
  const { selected, setView, setChat, user } = useApp();
  const p = selected;
  const [liked, setLiked] = useState(false);
  if (!p) return null;
  ...
  return (
    <div style={{ animation:"slideUp .2s ease", paddingBottom:90, background:"#FFFBF7", minHeight:"100vh" }}>
      ...
      <div style={{ position:"fixed", bottom:0, left:0, right:0, padding:"12px 16px", background:"#fff", borderTop:"1px solid #F0EBE5", display:"flex", gap:10, maxWidth:430, margin:"0 auto", boxShadow:"0 -4px 20px rgba(0,0,0,.06)" }}>
        ...
        <button className="press" onClick={()=>{ setChat({ name:p.seller||"卖家", av:(p.seller||"卖")[0], productId:p.id, productTitle:p.title, sellerId:p.user_id, buyerId:user?.id }); setView("chat"); }} style={{
...
function ChatView() {
  ...
  if (!user) return (
    <div style={{ height:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", background:"#FFFBF7", padding:32 }}>
      ...
      <button onClick={()=>setView("home")} style={{ marginTop:12, fontSize:14, color:"#78716C", background:"none", border:"none" }}>← 返回</button>
    </div>
  );
    ...
    const load = async () => {
      ...
      if (!cancelled && data) {
        const filtered = data.filter(m =>
          !m.buyer_id || m.buyer_id === buyerId || m.from_user === buyerId || m.from_user === user.id
        );
        setMsgs(filtered);
      }
    };
  ...
  const handleSend = async () => {
    ...
    if (!supabase) { setErrMsg("数据库未连接"); return; }
    setSending(true); setErrMsg(""); setInput("");
    const tempId = "t" + Date.now();
    setMsgs(m => [...m, { id:tempId, from_user:user.id, content:text, created_at:new Date().toISOString() }]);
    const payload = { from_user:user.id, product_id:productId, content:text, sender_name:myName };
    if (chat?.sellerId) payload.to_user = chat.sellerId;
    if (buyerId) payload.buyer_id = buyerId;
    const { data, error } = await supabase.from("messages").insert([payload]).select().single();
    setSending(false);
    if (error) { setErrMsg("发送失败：" + error.message); setMsgs(m => m.filter(x => x.id !== tempId)); setInput(text); }
    else if (data) { setMsgs(m => m.map(x => x.id === tempId ? data : x)); }
  };

  return (
    <div style={{ height:"100vh", display:"flex", flexDirection:"column", background:"#FFFBF7", overflow:"hidden" }}>
      <div style={{ padding:"44px 16px 14px", background:"#fff", borderBottom:"1px solid #F0EBE5", display:"flex", alignItems:"center", gap:12, flexShrink:0 }}>
        <button onClick={()=>setView("detail")} style={{ fontSize:22, background:"none", border:"none", cursor:"pointer" }}>←</button>
        <div style={{ width:40, height:40, borderRadius:"50%", background:OR, display:"flex", alignItems:"center", justifyContent:"center", color:"#fff", fontWeight:800, fontSize:17, fontFamily:"Syne,sans-serif", flexShrink:0 }}>{(chat?.name||"?")[0]}</div>
        <div style={{ flex:1, minWidth:0 }}>
          <p style={{ fontWeight:700, fontSize:15 }}>{chat?.name||"卖家"}</p>
          <p style={{ fontSize:11, color:"#78716C" }}>{chat?.productTitle ? `📦 ${chat.productTitle}` : "商品咨询"}</p>
        </div>
      </div>
      <div style={{ flex:1, overflowY:"auto", padding:"16px", display:"flex", flexDirection:"column", gap:12 }}>
        {msgs.length===0 && <div style={{ textAlign:"center", padding:"50px 0", color:"#78716C" }}><p style={{ fontSize:40 }}>👋</p><p style={{ fontSize:14, marginTop:12 }}>向卖家打招呼吧！</p></div>}
        {msgs.map((m, i) => {
            ...
            <div key={m.id||i} style={{ display:"flex", flexDirection:"column", alignItems:isMe?"flex-end":"flex-start" }}>
              <div style={{ display:"flex", alignItems:"flex-end", gap:8, flexDirection:isMe?"row-reverse":"row" }}>
                <div style={{ width:32, height:32, borderRadius:"50%", background:isMe?"#6366f1":OR, display:"flex", alignItems:"center", justifyContent:"center", color:"#fff", fontWeight:700, fontSize:13, flexShrink:0 }}>
                  {isMe ? myName[0].toUpperCase() : (chat?.name||"?")[0]}
                </div>
                <div style={{ maxWidth:"68%", padding:"10px 14px", fontSize:14, lineHeight:1.6, wordBreak:"break-word", borderRadius:isMe?"18px 18px 4px 18px":"18px 18px 18px 4px", background:isMe?OR:"#fff", color:isMe?"#fff":"#18181B", boxShadow:"0 2px 8px rgba(0,0,0,.08)" }}>{m.content}</div>
              </div>
              <p style={{ fontSize:10, color:"#9ca3af", marginTop:3, paddingLeft:isMe?0:40, paddingRight:isMe?40:0 }}>
                {m.created_at ? new Date(m.created_at).toLocaleTimeString("zh-CN",{hour:"2-digit",minute:"2-digit"}) : ""}
              </p>
            </div>
          );
        })}
        {errMsg && <p style={{ textAlign:"center", fontSize:12, color:"#ef4444", padding:"8px 16px", background:"#fef2f2", borderRadius:8 }}>❌ {errMsg}</p>}
        <div ref={endRef} />
      </div>
      <div style={{ background:"#fff", borderTop:"1px solid #F0EBE5", padding:"12px 16px 28px", display:"flex", gap:10, alignItems:"center", flexShrink:0 }}>
        <input
          value={input}
          ...
          placeholder="发消息给卖家..."
          disabled={sending}
          style={{ flex:1, padding:"12px 16px", borderRadius:24, border:"1.5px solid #F0EBE5", outline:"none", fontSize:14, background:"#fff" }}
        />
        <button onClick={handleSend} disabled={!input.trim()||sending}
          style={{ width:46, height:46, borderRadius:"50%", border:"none", cursor:input.trim()&&!sending?"pointer":"not-allowed", background:input.trim()&&!sending?OR:"#e5e7eb", color:"#fff", fontSize:18, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
          {sending?"…":"↑"}
        </button>
      </div>
    </div>
...
export default function App() {
  ...
  const addProduct = async p => {
    if (supabase) {
      const dbPayload = {
        title: p.title, desc: p.desc, price: p.price, cat: p.cat,
        loc: p.loc, cond: p.cond, img: p.img, seller: p.seller,
        status: p.status, user_id: p.user_id || null,
      };
