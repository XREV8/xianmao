...
function MessagesShell() {
  ...
  const loadConvos = async (showLoad = true) => {
    if (!supabase || !user) { setLoading(false); return; }
    if (showLoad) setLoading(true);

    // 1. 我发的消息（作为买家）
    const { data: sent } = await supabase
      .from("messages").select("*").eq("from_user", user.id)
      .order("created_at", { ascending: false });

    // 2. 我商品收到的消息（作为卖家）
    const { data: myProds } = await supabase
      .from("products").select("id, title, seller, user_id").eq("user_id", user.id);
    let received = [];
    if (myProds?.length > 0) {
  ...
  useEffect(() => {
    loadConvos(true);
    const interval = setInterval(() => loadConvos(false), 5000);
    ...
    <div style={{ paddingBottom:"calc(60px + 12px)", background:"#FFFBF7", minHeight:"100vh" }}>
      <div style={{ padding:"44px 16px 16px", background:OR, display:"flex", justifyContent:"space-between", alignItems:"flex-end" }}>
        <div>
          ...
          <p style={{ fontSize:13, color:"rgba(255,255,255,.75)", marginTop:4 }}>所有对话记录</p>
        </div>
        <button onClick={()=>loadConvos(false)} style={{ color:"rgba(255,255,255,.8)", fontSize:20, background:"none", border:"none" }}>🔄</button>
      </div>
      ...
      <div style={{ padding:"12px 16px", display:"flex", flexDirection:"column", gap:10 }}>
        {convos.map(c => {
          ...
          const otherName = iAmSeller ? (c.sender_name || "买家") : (product?.seller || "卖家");
          const isUnread = c.from_user !== user.id && !c.read;
          const buyerIdForChat = c.buyer_id || c.from_user;

          return (
            <div key={`${c.product_id}_${buyerIdForChat}`} className="hover-card"
              onClick={() => {
                setChat({
                  name: otherName,
                  av: otherName[0],
              ...
              style={{ background:"#fff", borderRadius:14, padding:"14px 16px", boxShadow:"0 2px 8px rgba(0,0,0,.06)", display:"flex", alignItems:"center", gap:12, cursor:"pointer", border: isUnread?"1.5px solid #fed7aa":"1.5px solid transparent" }}>
              <div style={{ width:48, height:48, borderRadius:"50%", background: isUnread?OR:"#e5e7eb", display:"flex", alignItems:"center", justifyContent:"center", color: isUnread?"#fff":"#78716C", fontWeight:800, fontSize:18, fontFamily:"Syne,sans-serif", flexShrink:0 }}>
                {otherName[0]}
              </div>
              <div style={{ flex:1, minWidth:0 }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                  <p style={{ fontSize:14, fontWeight: isUnread?700:600 }}>{otherName}</p>
                  <p style={{ fontSize:11, color:"#78716C" }}>{new Date(c.created_at).toLocaleTimeString("zh-CN",{hour:"2-digit",minute:"2-digit"})}</p>
                </div>
                <p style={{ fontSize:12, color: isUnread?"#18181B":"#78716C", marginTop:3, overflow:"hidden", whiteSpace:"nowrap", textOverflow:"ellipsis", fontWeight: isUnread?600:400 }}>
            ...
            <>
              <div style={{ position:"relative", display:"inline-block" }}>
                ...
                {it.badge && unreadCount > 0 && (
                  <span style={{ position:"absolute", top:-4, right:-6, minWidth:16, height:16, borderRadius:8, background:"#ef4444", color:"#fff", fontSize:10, fontWeight:700, display:"flex", alignItems:"center", justifyContent:"center", padding:"0 4px" }}>{unreadCount > 99 ? "99+" : unreadCount}</span>
                )}
              </div>
...
function ChatView() {
  ...
  const loadMsgs = async (showLoad = false) => {
    ...
    if (data) {
      setMsgs(data);
      // 标记已读
      const unread = data.filter(m => m.from_user !== user.id && !m.read);
      if (unread.length > 0) {
        supabase.from("messages").update({ read: true }).in("id", unread.map(m => m.id));
      }
    }
    if (showLoad) setLoading(false);
  };
  ...
  useEffect(() => {
    ...
    loadMsgs(true);
    // 每3秒轮询
    const interval = setInterval(() => loadMsgs(false), 3000);
    // Realtime
    let sub;
    try {
      sub = supabase?.channel(`msg_${productId}_${buyerId}`)
        .on("postgres_changes", { event:"INSERT", schema:"public", table:"messages", filter:`product_id=eq.${productId}` }, () => loadMsgs(false))
        .subscribe();
    } catch(e) {}
    return () => { clearInterval(interval); if(sub) supabase?.removeChannel(sub); };
  }, [productId, buyerId]);
  ...
  const send = async () => {
    ...
    // 乐观显示
    const tempMsg = { ...newMsg, id: `temp_${Date.now()}`, created_at: new Date().toISOString() };
    setMsgs(m => [...m, tempMsg]);
    ...
    if (error) {
      ...
      setMsgs(m => m.map(x => x.id === tempMsg.id ? data : x));
    }
    setSending(false);
  ...
  return (
    <div style={{ height:"100vh", display:"flex", flexDirection:"column", background:"#FFFBF7" }}>
      {/* 顶部栏 */}
      <div style={{ padding:"44px 16px 12px", background:"#fff", borderBottom:"1px solid #F0EBE5", display:"flex", alignItems:"center", gap:12, flexShrink:0 }}>
        <button onClick={()=>setView("detail")} style={{ fontSize:22, lineHeight:1 }}>←</button>
        <div style={{ width:40, height:40, borderRadius:"50%", background:OR, display:"flex", alignItems:"center", justifyContent:"center", color:"#fff", fontWeight:800, fontSize:17, fontFamily:"Syne,sans-serif", flexShrink:0 }}>
          {(chat?.av || "?")[0]}
        </div>
        <div style={{ flex:1, minWidth:0 }}>
          <p style={{ fontWeight:700, fontSize:15, overflow:"hidden", whiteSpace:"nowrap", textOverflow:"ellipsis" }}>{chat?.name || "卖家"}</p>
          <p style={{ fontSize:11, color:"#78716C", overflow:"hidden", whiteSpace:"nowrap", textOverflow:"ellipsis" }}>
            {chat?.productTitle ? `📦 ${chat.productTitle}` : "商品咨询"}
          </p>
        </div>
      </div>
      ...
      <div style={{ flex:1, overflowY:"auto", padding:"16px", display:"flex", flexDirection:"column", gap:12 }}>
        {loading && (
          <p style={{ textAlign:"center", color:"#78716C", fontSize:13, padding:"20px 0" }}>加载消息中…</p>
        )}
        {!loading && msgs.length === 0 && (
          <div style={{ textAlign:"center", padding:"40px 0", color:"#78716C" }}>
            <p style={{ fontSize:36 }}>👋</p>
            <p style={{ fontSize:14, marginTop:10, fontWeight:500 }}>向卖家打个招呼吧！</p>
            <p style={{ fontSize:12, marginTop:4 }}>发消息开始聊天</p>
          </div>
        )}
        {msgs.map((m, i) => {
          ...
          return (
            <div key={m.id || i} style={{ display:"flex", flexDirection:"column", alignItems:isMe?"flex-end":"flex-start" }}>
              <div style={{ display:"flex", alignItems:"flex-end", gap:8, flexDirection:isMe?"row-reverse":"row" }}>
                {/* 头像 */}
                <div style={{ width:32, height:32, borderRadius:"50%", background:isMe?"#6366f1":OR, display:"flex", alignItems:"center", justifyContent:"center", color:"#fff", fontWeight:700, fontSize:13, flexShrink:0 }}>
                  {isMe ? myName[0].toUpperCase() : (chat?.av||"?")[0]}
                </div>
                {/* 气泡 */}
                <div style={{ maxWidth:"68%", padding:"10px 14px", fontSize:14, lineHeight:1.6, borderRadius:isMe?"18px 18px 4px 18px":"18px 18px 18px 4px", background:isMe?OR:"#fff", color:isMe?"#fff":"#18181B", boxShadow:"0 2px 8px rgba(0,0,0,.08)", wordBreak:"break-word" }}>
                  {m.content}
                </div>
              </div>
              <p style={{ fontSize:10, color:"#78716C", marginTop:4, paddingLeft:isMe?0:40, paddingRight:isMe?40:0 }}>
      ...
      {/* 输入框 */}
      <div style={{ padding:"10px 16px 20px", background:"#fff", borderTop:"1px solid #F0EBE5", display:"flex", gap:10, alignItems:"center", flexShrink:0 }}>
        <input
          ref={inputRef}
          value={input}
          ...
          placeholder="发消息给卖家..."
          style={{ flex:1, padding:"12px 16px", borderRadius:24, border:"1.5px solid #F0EBE5", outline:"none", fontSize:14, background:"#fafaf9", WebkitAppearance:"none" }}
        />
        <button
          onClick={send}
          disabled={!input.trim() || sending}
          style={{ width:46, height:46, borderRadius:"50%", background:input.trim()&&!sending?OR:"#e5e7eb", color:"#fff", fontSize:20, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, border:"none", cursor:input.trim()&&!sending?"pointer":"not-allowed", boxShadow:input.trim()&&!sending?"0 3px 10px rgba(249,115,22,.4)":"none" }}
        >
          {sending ? "…" : "↑"}
        </button>
      </div>
